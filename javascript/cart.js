...
$('#confirm-button-next').click(function (e) { //нажатие кнопки далее
    ...
    if (!$(".order_delivery").is(":checked")) { // выбран ли способ доставки не выбран
        $(".label_delivery").css("color", "rgb(169, 0, 0)"); //делаем шрифт красным
        ...
        $("#order_address").css("borderColor", "rgb(255, 255, 255)"); //делаем шрифт белым

        // проверка фамилии
        if (order_fio.length < 3 || order_fio.length > 20) {
            $("#order_fio").css("borderColor", "rgb(169, 0, 0)"); //делаем рамку красным
            send_order_fio = '0'; //переменная для определения допущена ли ошибка

        } else {
            $("#order_address").css("borderColor", "rgb(0, 0, 0)"); //делаем рамку черной
            send_order_fio = '1'; //переменная для определения не допущена ли ошибка
        }

        // проверка имени
        if (order_name.length < 3 || order_name.length > 15) {
            $("#order_name").css("borderColor", "rgb(169, 0, 0)"); //делаем рамку красным
        ...
        if (order_patronymic.length < 3 || order_patronymic.length > 25) {
            $("#order_patronymic").css("borderColor", "rgb(169, 0, 0)"); //делаем рамку красным
        ...
        if (isValidEmailAddress(order_email) == false) {
            $("#order_email").css("borderColor", "rgb(169, 0, 0)"); //делаем рамку красным
        ...
        if (order_phone.length < 1 || order_phone.length > 12) {
            $("#order_phone").css("borderColor", "rgb(169, 0, 0)"); //делаем рамку красным
        ...
        if (order_address.length < 1 || order_address.length > 150) {
            $("#order_address").css("borderColor", "rgb(169, 0, 0)"); //делаем рамку красным
